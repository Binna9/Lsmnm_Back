...
function fc_includeTitle() {
    ...
    try {
        ...
        // custom button�� text�� �ъ씠利덈� 媛��몄��� width �ㅼ젙
    ...
    try {
        ...
        try {
            $.each( cboList , function( iloop, cboData ) {
                ...
                // 2022.04.21 Revision by. MH_PROC_LOV, MG_DEPT_CD 遺��� 諛� 怨듭젙 肄붾뱶瑜� �붾㈃蹂꾨줈 援щ텇�섍린 �꾪븿. Start
                paramData[ 'MES_GW_PGM_ID' ] = window.gwMesEnv.user.pgmId;
                // 2022.04.21 Revision by. MH_PROC_LOV, MG_DEPT_CD 遺��� 諛� 怨듭젙 肄붾뱶瑜� �붾㈃蹂꾨줈 援щ텇�섍린 �꾪븿. End
            ...
            // 새로운 combo API 사용 (동기 방식으로 변경)
            var codeData = JSON.stringify( { CODE: obj } );
            ...
            $.ajax({
                ...
                success: function(response) {
                    try {
                        // 응답이 문자열인 경우 JSON 파싱
                        var jsonResult = typeof response === 'string' ? JSON.parse(response) : response;
                        
                        // 기존 방식과 동일하게 window.gwJsonResult에 저장
                        window.gwJsonResult = jsonResult;
