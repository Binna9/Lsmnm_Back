...
//                okButton: $('#OK_BTN'),
        ...
        function _createGrid( targetId, colNm ) {       	
			...
			var sItemCd  = '';
			var objItems = [
                        { caption: 'Code'		  , datatype: 'text', align: 'center', name: 'CD_VAL'     , width: '30%', hidden: true },
                        { caption: 'Code Name'	  , datatype: 'text', align: 'center', name: 'CD_NM'      , width: '35%', hidden: true },
                        { caption: 'Remarks'	  , datatype: 'text', align: 'center', name: 'REMARKS'    , width: '40%', hidden: true },
                        { caption: 'hidden value1', datatype: 'text', align: 'center', name: 'HIDDEN_VAL1', width: '0'  , hidden: true },
                        { caption: 'hidden value2', datatype: 'text', align: 'center', name: 'HIDDEN_VAL2', width: '0'  , hidden: true },
	...
	var _returnSelected = function() {
		...
		if ( !fc_isNull( rowid ) && !fc_isNull( colNm ) ) {
			var gridDifFlag = fc_getCellData( targetId, rowid, colNm ) != rowData.CD_VAL ? true : false;
...
function fc_makePopupDiv( targetid, colNm ) {
	...
	var divButton    = $( "<div id='popupWindowButton' style='margin: 10px 10px 10px 10px;'><div style='float: right;'><input type='button' id='POP_OK_BTN' value='" + window.gwMessage.button.bOk + "' style='margin-right: 10px' /><input type='button' id='CANCEL_BTN' value='" + window.gwMessage.button.bCancel + "' style='margin-right: 10px' /></div></div>" );
	...
	for ( var i=0;i<cap.length;i++ ) {
		if ( !fc_isNull( cap[ i ] ) ) {
			...
			var input = $( "<input type='text' style='width: 120px; border: 1px solid #aaa'/>" );
