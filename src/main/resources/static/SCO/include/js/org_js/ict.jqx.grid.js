 ...
 * @param data
 */
function fc_setGridlocalData( gridId, data ) {
	var obj = $( '#' + gridId );
	if ( fc_isNull( obj ) ) {
...
//		$( '#' + gridId ).jqxGrid( 'beginupdate' );
		for ( var loop=0;loop<arrColNm.length;loop++ ) {
			$( '#' + gridId ).jqxGrid( 'hidecolumn', arrColNm[ loop ] );
		};
...
//		$( '#' + gridId ).jqxGrid( 'beginupdate' );
		for ( var loop=0;loop<arrColNm.length;loop++ ) {
			$( '#' + gridId ).jqxGrid( 'showcolumn', arrColNm[ loop ] );
		};
//		$( '#' + gridId ).jqxGrid( 'endupdate' );
	} catch ( e ) {
		fc_getException( e );
...
function fc_setGridCombo( gridId, colNm, arrGridData, listCount ) {
	try {
		...
		var heightSize = arrGridData.length * 24;

		if (!fc_isNull(listCount)) {
			heightSize = listCount * 24;
		};
