...
window.gwMesEnv = {
	...
	user       : { id: 'LOCAL', name: 'UNKNOWN', deptCd: 'UNKNOWN', clientIp: 'LOCAL PC', pgmId: '', pgmTitle: '', pgmTy: '', comCd: 'POSCOICT', comNm: 'Posco ICT', plantCd: 'K123' },
	...
	btnSize   : { search: 85, save: 85, del: 85, conf: 85, print: 85, close: 85, cust1: 100, cust2: 100, cust3: 100, cust4: 100, cust5: 100 },
};
...
function fc_makeMessageBoxDiv ( msg, msgType, msgDetails ) {
	...
	var divSubContents = $( "<div id='msgBoxSubContents'><table><tr><td class='" + msgClass + "'><span></span></td><td class='content_text'>" + msg + "</td></tr></table></div>" );
...
//	errMessage = errMessage.replace( /\s*$/gi, '' );
	var C_REGEXP_ORA_ERR = /ORA-[\d]{5}:/g;
	var rtnValue = '';
 ...
 * @returns {Boolean}
 */
function fc_submit_file ( type, prmServiceNm, prmTransitionNm, prmParam, prmDataType, prmModule ) {
	var result = false;
	// set default parameters
	fc_setDefaultParam( prmServiceName );
...
function fc_getDefPgmId () {
	var arrCurrentUrl = document.location.href.split( '?' );
	...
	sPgmId = currentUrl.substring( currentUrl.lastIndexOf( '/' ) + 1, currentUrl.indexOf( '.do' ) );
...
function fc_progress ( bFlag, progMsg ) {
	...
	if ( window.isProgressRun == bFlag ) return;
	if ( window.isAjaxRun == true ) return;
	var progDialogId   = 'divGwProgDialog';
	var progressId = 'divGwProgress';
	$ ( 'body' ).enabled = !bFlag;
...
function fc_setGridData( arrTargetModel ) {
	try {
		...
		$.each( arrTargetModel, function( iloop, modelData ) {
			...
			fc_setGridlocalData( modelData.gridId, window.gwJsonResult[ modelData.resultKey.trim() ] );
