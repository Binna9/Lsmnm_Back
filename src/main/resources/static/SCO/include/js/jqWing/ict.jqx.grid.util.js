...
var number5Template		= { type: 'number', align: 'right' , aggregates: ['sum']  , editable: true, sortable: true, pinned: false, nullable: true, filtertype: 'number', cellformat:'d5' };
...
//	window.gwMultiLang.isRunMain = false;
	...
	try {
		...
		var createRownum = !fc_isNull(option) && !option['rownum'] ? false : true;  //2020.4.23
		var createPageable = !fc_isNull(option) && !option['pageable'] ? false : true; //2020.4.23
		...
		var _setColumnProp = function( columnObj , colProperty, displayObj ) {
			try {
				...
				if ( columnObj.datatype == 'popup' || columnObj.datatype == 'custpopup' ) {
					...
					//KD 1105 커스텀 popup type 추가
					propObj.poptype = ( columnObj.popType == null ) ? 'search' : columnObj.popType;
				} else {
					propObj.popuptype = null;
				};
				
				var inBrClass = '';
				if( typeof propObj.text == 'string'){
					if(propObj.text.indexOf('<br>') > -1 || propObj.text.indexOf('<br/>') > -1){
						inBrClass = 'in_br';
					}
				}
					...
					if ( propObj.isMultiLanguage ) {
						...
						if ( window.gwMultiLang.isRunMain ) {
							...
							propObj.rendered = function ( columnHeaderElement ) {
								if(!fc_isNull( inBrClass )){
									columnHeaderElement.addClass( inBrClass );
								}
		...
		var _createPagerRender = function () {
			try {
                ...
                this.pagereloadbtn = this.pagereloadbtn || $( "<div style='float: left;'><div style='margin-top: 0px; width: 19px; height: 17px; background-size: 19px 17px;'></div></div>" );
                this.pageaddbtn    = this.pageaddbtn    || $( "<div style='float: left;'><div style='margin-top: 0px; width: 19px; height: 17px; background-size: 19px 17px;'></div></div>" );
                this.pagecopybtn   = this.pagecopybtn   || $( "<div style='float: left;'><div style='margin-top: 0px; width: 19px; height: 17px; background-size: 19px 17px;'></div></div>" );
                this.pagedeletebtn = this.pagedeletebtn || $( "<div style='float: left;'><div style='margin-top: 0px; width: 19px; height: 17px; background-size: 19px 17px;'></div></div>" );
                ...
                this.addHandler( this.pageaddbtn, 'click', function() {
                    if ( !me.pageaddbtn.jqxButton( 'disabled' ) ) {
                    	try {
                    		...
                    		if(cell != null){
                    			var editable = $("#"+gridId).jqxGrid('endcelledit', cell.row, cell.datafield, false);
                    		}
...
//	        altrows: false,		 		// Row 강조
	        ...
	        sortable: tySort,				// sort 필수
	        showsortmenuitems: false,
	        showsortcolumnbackground: false,	// sort 배경 색상 유무
	        showpinnedcolumnbackground: false,	// 틀고정 색상 유무
	        selectionmode:(isMultiSelect == true) ? 'singlecell' : 'singlerow', //'multiplecellsadvanced'
	        keyboardnavigation: isMultiSelect,
//	        autoloadstate: false,
//	        autosavestate: false,
...
//	        toolbarheight: 26,
		...
		$.each( arrColPopup, function( index, item ) {
			var searchPopupId = "search_" + item.datafield + gridId;
						
			var _createPopup = function( row, cellvalue, editor, cellText, width, height ) {
				try {
					var maxlengthths = this.owner.columnmaxlength;
					var inputElement;//$( "<div style='jqx-max-size jqx-position-relative'><input type='text' /><div id='" + searchPopupId + "'><img alt='" + searchPopupId + "' width='16' height='16' src='./include/images/search_lg.png' /></div></div>" ).prependTo( editor );
					//KD 커스텀 Popup 이미지 확장 추가
					if(arrColPopup[index].poptype == "search"){
						inputElement = $( "<div style='jqx-max-size jqx-position-relative'><input type='text' style='border:none;'/><div id='" + searchPopupId + "'><img alt='" + searchPopupId + "' width='16' height='16' src='./include/images/search_lg.png' /></div></div>" ).prependTo( editor );
					}else if(arrColPopup[index].poptype  == "writeL") {
						inputElement = $( "<div style='jqx-max-size jqx-position-relative'><input type='text' readonly style='border:none;'/><div id='" + searchPopupId + "'><img alt='" + searchPopupId + "' width='16' height='16' src='./include/images/write_lg.png' /></div></div>" ).prependTo( editor );
					}
					//~ KD
			...
			var _initPopup = function( row, cellvalue, editor, celltext, pressedkey ) {
				try {
					...
					if ( item.popuptype == 'POP' ) {
						...
						//세로 스크롤이 있는 그리드인 경우 발생, 데이터 조회 후 첫번째 행 클릭시 'initeditor'중복 발생으로 'click' 이벤트 중복처리로 인한 오류발생하여 임시 조치.
						//추후 정확한 원인 분석 후 추가 조치 필요.
						if( row == 0 ){
							$( '#' + searchPopupId ).unbind( 'click' );
						}
						$( '#' + searchPopupId ).click( function() {
							try {
...
//						break;
	     ...
	     grid.bind( 'cellendedit', function( event ) {
	    	 try {
	    		 ...
	    		 //console.log("################ grid cellendedit ", event);
	    		 
	    		 // 2020-03-20
	    		 // 그리드 LOV edit시 체크박스 설정 추가
	    		 // 2020-03-30
	    		 // 그리드 datetimeinput edit시 체크박스 설정 추가
	    		 if(event.args.columntype == "dropdownlist"){
	    			 cellValue = fc_setValue(event.args.value.value, '' );
		    		 cellOldValue = fc_setValue(event.args.oldvalue.value, '' );
	    		 }else if(event.args.columntype == "datetimeinput"){
	    			 
	    			 if(event.args.value == null) return;
	    			 
	    			 cellValue = event.args.value.toString();
	    			 
	    			 if(event.args.oldvalue == null){
	    				 cellOldValue = "";
	    			 }else{
	    				 cellOldValue = event.args.oldvalue.toString();
	    			 }
	    			 
		    	 }else{
	    		 	 cellValue = fc_setValue( event.args.value, '' );
		    		 cellOldValue = fc_setValue( event.args.oldvalue, '' );
	    		 }
		    	 ...
		    	 if ( event.args.datafield == 'JQX_CB' ) {
		    		 ...
		    		 //var rowStatus = $( '#' + gridId ).jqxGrid( 'getcellvalue', event.args.rowindex, 'JQX_RS');
		    		 ...
		    		 if ( column.displayfield != column.datafield ) {
		    			 if ( cellValue != cellOldValue ) {
		            		 ...
		            		 // JQX_RS 20191118
		            		 // 데이터 수정시 JQX_CB=true 시 JQX_RS='U' 저장
		            		 //if(rowStatus != "D" && rowStatus != "I"){
		            		 //	 $( '#' + gridId ).jqxGrid( 'setcellvalue', event.args.rowindex, 'JQX_RS', window.gwMesEnv.grid.updateFlag );
		            		 //}
		    			 }
		    		 } else if ( cellValue != cellOldValue ) {
		        		 ...
		        		 // number형일경우 현재값과 이전값의 타입이 다름.
		        		 // datatype이 존재하지 않아 filtertype으로 대체
		        		 if(column.filtertype == 'number'){
			        		 if(typeof cellValue != typeof cellOldValue && typeof cellValue == 'object' && typeof cellOldValue == 'string'){
			        			 
			        			 if(Number(cellValue) == 0 && cellOldValue.trim() == ''){
...
*/
	...
	*/
	...
	//console.log('######## EXPORT_DATA instance ', instance);
	
	// 20200427
	// Grid 표시된 데이터 그대로 Excel 출력
	//sendData[ 'EXPORT_DATA' ] = instance.source._source.localdata;
	sendData[ 'EXPORT_DATA' ] = instance.source.records;
	...
	try {
		...
		var _cellvaluechanged = function( event ) {
			try {
			    ...
			    var value         = event.args.newvalue == null ? '' : event.args.newvalue ;
...
/*
 * function fc_mergeGridCol
 * custom merge function 2019.11.1
 * position : top,center,buttom
 */
function fc_mergeGridCols(gridId, mergeInfoArr, orgData){
     gvLineHideInfo = null;
     gvLineHideInfo = new Object();
     
     gvTextHideInfo = null;
     gvTextHideInfo = new Object();
    
     $.each(mergeInfoArr, function(index, mergeInfo){
         fc_mergeGridCol(gridId, mergeInfo.colId, mergeInfo.parentColId, mergeInfo.childColId, mergeInfo.position, orgData);
     });
     
     gvInfo[gridId] = {'gvLineHideInfo':gvLineHideInfo, 'gvTextHideInfo':gvTextHideInfo};
}

function fc_mergeGridCol(gridId, colId, parentColId, childColId, position, orgData){
	var chkParent = !fc_isNull(parentColId) ? true : false;
    var gridTypeIndex = fc_getColIndexbyName2(gridId, 'JQX_CB') == -1 ? 0 : 1;
    var colIndex = fc_getColIndexbyName2(gridId, colId)+1+gridTypeIndex;// +1: rowNum +1,checkbox
    var colType = fc_getColTypebyName(gridId, colId);
    //console.log('#'+colId+':'+colIndex+'/'+colType);
    
    if(fc_isNull(orgData)){
        console.log('no data : '+colId) 
        return;
    }
    if(fc_isNull(colIndex)){
        console.log('no column index : ' + colId) 
        return;
    }
    
    if(fc_isNull(position)){
        position = 'top';
    }
    ...
    var gridCount = orgData.length;
    
    var curVal = null;
    var beforeVal = null;
    var curValParent = null;
    var beforeValParent = null;
    ...
    $.each(orgData, function(index, data){
        curVal = data[colId];
        if(chkParent){
        	curValParent = data[parentColId];
        }
        ...
        if(beforeVal != null){
            ...
            if(curVal != beforeVal){
                if(startIndex != null  && endIndex != null && endIndex != startIndex){
                    groupArr.push({groupid:groupIndex++, start:startIndex, end:endIndex});      
                    startIndex = null;
                    endIndex = null;
                }
            }else{
            	if(chkParent && curValParent != beforeValParent ){//&& fc_isNull(beforeValParent)
            		if(startIndex != null  && endIndex != null && endIndex != startIndex){
                        groupArr.push({groupid:groupIndex++, start:startIndex, end:endIndex});      
                        startIndex = null;
                        endIndex = null;
        ...
        //console.log('## 33: ' + index + ' : '+ curVal + '/' + beforeVal + ' || ' + startIndex+'/'+endIndex);
        beforeVal = curVal;
        
        if(chkParent){
        	beforeValParent = curValParent;
        }
        
    })
    
    //disp text posision
    $.each(groupArr, function(index, data){
        switch(position){
        case 'top':
            data.dispcol = data.start; break;
        case 'center':        	
            data.dispcol = data.start + Math.trunc((data.end-data.start)/2);
            break;
        case 'bottom':
            data.dispcol = data.end; break;
        default:
            data.dispcol = data.start; break;
        }
    });
    
    //console.log('## groupArr : '+ position + ' / '+ JSON.stringify(groupArr));
    
    //draw
    var lineHideObj = new Object();
    var TextHideObj = new Object();
    $.each(groupArr, function(index, data){
        for(var i=data.start; i<=data.end; i++){
            //line control
            if(i < data.end){
                lineHideObj[i] = true;//global variable for cellclassname
            }
            
            //disp text coltrol
            if(i != data.dispcol){
                TextHideObj[i] = true;//global variable for cellclassname
            }
            
        }
    });
    
    gvLineHideInfo[colId] = lineHideObj;
    gvTextHideInfo[colId] = TextHideObj;

    //line control
    var cellClass = fc_getColProp( gridId, colId, 'cellclassname');
    if(String(cellClass).indexOf('f_mergeRowClass') == -1 ){
        fc_setColProp( gridId, colId, 'cellclassname', f_mergeRowClass );
        //console.log('## fc_setColProp ['+colId+']' + String(cellClass));
    }
    
}

function fc_getColIndexbyName2( gridId, colNm ) {
    try {
        var cm = fc_getGridColModel( gridId );
        var loopCnt = 0;
        for ( var i=0;i<cm.length;i++ )
            
            if(cm[ i ].name.indexOf('DISP_') > -1){
                continue;
            }else{
                if ( cm[ i ].name === colNm.toUpperCase() ) return loopCnt;
                loopCnt++;
            }
...
function fc_getColTypebyName( gridId, colNm ) {
    try {
        var cm = fc_getGridColModel( gridId );
        var loopCnt = 0;
        for ( var i=0;i<cm.length;i++ ){
            if ( cm[ i ].name === colNm.toUpperCase() ) return cm[ i ].datatype;
        }
...
var gvInfo = {};
var gvLineHideInfo = null;
var gvTextHideInfo = null;
var gvIsReadOnlyCell = false;
var existClass = null;
var returnClass = null;
function f_mergeRowClass(row, datafield, value, data){
    if ( !this.editable ) {
        existClass = window.gwClass.notGridEditClass;
    }else if( !this.nullable) {
        existClass = window.gwClass.required;
    };

    returnClass = existClass;
    
    try{
    	var mgGridId = this.owner.dataview.grid.content.context.id;
    	
    	if(gvInfo[mgGridId].gvLineHideInfo[datafield][row] != null){
            returnClass += ' mergeCol';         
        }
        
        if(gvInfo[mgGridId].gvTextHideInfo[datafield][row] != null){
            returnClass +=' mergeHideText';
        }
	}catch(e){
		console.log("f_mergeRowClass.......error....");
	}finally{
...
/*
 * function fc_mergeGridRow
 * custom merge function 2019.11.1
 * position : top,center,buttom
 */
function fc_mergeGridRows(){
     //todo
}
...
//조회화면에서 영역선택 복사 -> excel붙여넣기 가능하도록 하는 옵션
function fc_setGridRangeCopyEnable(gridId){
	$("#"+gridId).jqxGrid('enablebrowserselection', false);
	$("#"+gridId).jqxGrid('selectionmode', 'multiplecellsextended');//multiplerowsadvanced
	$("#"+gridId).jqxGrid('keyboardnavigation', true);//multiplerowsadvanced
	$("#"+gridId).jqxGrid('clipboard', true);//multiplerowsadvanced
}

//화면내 붙여넣기 기능 활성화 (lov유형의 경우 label값으로 붙어넣기 필요) 
function fc_setDataPasteEnable(gridId){
	fc_setGridParam( gridId, { editmode: 'selectedcell', clipboard: true } );
}

//Grid의 기본 default Row 생성 2020.4.23
function fc_setGridDefaultRow(gridId, count, data){
	setTimeout(function(){
		var gridRowCnt = fc_getGridRecordCount(gridId);
		var loopCnt = count - gridRowCnt;

		if(loopCnt <= 0){
			return false;
		}
		
		var newRowSet = fc_getRowsData(gridId);
		if(fc_isNull(newRowSet) || newRowSet.length == 0 ){
			newRowSet = new Array()
		}
		...
		for(var i = 0; i < loopCnt; i ++){
			...
			data['JQX_RS'] = window.gwMesEnv.grid.addFlag;
			data['JQX_CB'] = false;
			newRowSet.push(rowObject);
			
		}
		
		fc_setGridlocalData( gridId, newRowSet);
