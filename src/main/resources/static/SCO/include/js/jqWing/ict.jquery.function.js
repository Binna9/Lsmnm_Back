...
function fc_getLineCd() {
	return window.gwMesEnv.user.lineCd;
}; // end of fc_getUserId
/**
 * fc_getGridFlag
 * @param sMode
...
function fc_makeSplitter( targetObjId, panelSize1, panelSize2, sOrientation ) {
	if ( sOrientation == 'H' || sOrientation == 'h' ) {
		$( '#' + targetObjId ).jqxSplitter( { width: 'calc( 100% - 2px )', height: 'calc( 100% - 1px)', panels: [ { size: panelSize1, min: '10%' }, { size: panelSize2, min: '10%' } ], orientation: 'horizontal' } );
		//$( '#' + targetObjId ).css('margin-top','1px');
		//$( '#' + targetObjId ).css('margin-bottom','1px');
		
	
	} else {
		$( '#' + targetObjId ).jqxSplitter( { width: 'calc( 100% - 2px )', height: 'calc( 100% - 2px )', panels: [ { size: panelSize1, min: '10%' }, { size: panelSize2, min: '10%' } ] } );
		//$( '#' + targetObjId ).css('margin-left','1px');
		//$( '#' + targetObjId ).css('margin-right','1px');
	};
	
	$( '#' + targetObjId ).css('border','0px');
 ...
 * @returns {jQueryGrid}
 */
function fc_makeGrid( targetObjId, type, key, subKey, caption_flag, sort_ty,attribute1, attribute2, tabGridFlag, addType, bGridMultiLanguage, option ) {
	try {
		key          = fc_setUpperValue ( key );
		subKey       = fc_setUpperValue ( subKey );
		caption_flag = fc_setNullToTrue ( caption_flag );
		targetObj    = fc_getObj ( targetObjId );
		sort_ty		 = (fc_isNull (sort_ty))? true : sort_ty;
		...
		var objGrid = new jQueryGrid( targetObj, gridId, objItems, type, gridCaption, gridKey, objGrpItems, tabGridFlag, addType, bGridMultiLanguage, sort_ty,option );
...
function fc_makeDOMSubTab( targetObj, tabsId, tabObjList, tabContentsObj, key, subKey, attribute1, attribute2 ) {
	...
	try {		
		...
		for ( var loop=0;loop<tabObjList.length;loop++ ) {
			var tabPage = tabObjList[ loop ];
			var tabPageId = tabPage.tabPageId;
			var tabPageObj = tabPage.tabPageObj;
			
			var contentsClass = '';
			if ( !fc_isNull( tabPage.contentstype ) ) {
				contentsClass = 'contents' + tabPage.contentstype;
			};

			var divId = tabPageId.toUpperCase();			
		    ...
		    if($('#'+divId).length > 0){
		    	...
		    	divContents = $( '<div id="' + divId + '" ' + ( fc_isNull( contentsClass ) ? '' : ( 'class="'+ contentsClass + '"' ) ) +'></div>' );
		    	isExist = false;
		    }
			
			var objTabContent = new Array();
			//tabPageObj
			$.each(tabPageObj, function (index, item) { 
				var objtype = item.objtype.toUpperCase();
				var targetObjId = item.targetObjId;
				var authority = item.authority;
				var key = item.key;
				var subKey = item.subKey;	
				var captionFlag = item.captionFlag || false;
				var sortTy = item.sortTy || true;
				var isTabGrid = item.isTabGrid || false;
				var addType = item.addType || true;
				var isMultiLanguage = item.isMultiLanguage || false;
				
				if ( objtype == 'GRID' ) {
					objTabContent[ index ] = fc_makeGrid(  
							  targetObjId
							, authority
							, key
							, fc_isNull( subKey ) ? null : subKey
							, captionFlag
							, sortTy
							, ( fc_isNull( attribute1 ) ? null : ( typeof attribute1 == 'string' ) ? attribute1 : attribute1[ index ] )
							, ( fc_isNull( attribute2 ) ? null : ( typeof attribute2 == 'string' ) ? attribute2 : attribute2[ index ] )
							, isTabGrid
							, addType
							, isMultiLanguage
							//, fc_setValue( item.addType, true )
							//, fc_setValue( item.isMultiLanguage, window.gwMesEnv.lang.isMultiLanguage ) 
							);
					
					if( !isExist ){
						$( '#' + objTabContent[ index ].GridId ).appendTo( divContents );
					}
					
				} else if ( objtype == 'FORM' ) {
					objTabContent[ index ] = fc_makeTable( targetObjId, key, subKey, captionFlag );
					
					if( !isExist ){
						divContents.append( objTabContent[ index ].object );						
					}
				}
				
				if ( typeof item.makeAfter == 'function' ) {
					item.makeAfter();
				};
				
			});	
			//tabPages push
			tabPages.push(objTabContent);
			
			tabPage.isMultiLanguage = fc_setValue( tabPage.isMultiLanguage, true );
			if ( window.gwMesEnv.lang.isMultiLanguage && tabPage.isMultiLanguage ) {
				var _itemcd  = ( fc_isNull( tabPage.itemCd    ) ) ? tabPage.tabPageId.toUpperCase() : tabPage.itemCd.toUpperCase().trim();
				var _itemval = ( fc_isNull( tabPage.itemValue ) ) ? 0 : tabPage.itemValue;
				if ( window.gwMultiLang.isRunMain ) {
					if ( !fc_isNull( _itemcd ) ) {
						tabPage.caption = fc_getGwLangItem( _itemcd, _itemval, tabPage.caption );
					};
				} else {
					if ( !fc_isNull( _itemcd ) ) {
			...
			if ( !fc_isNull( tabPage.tabPageId.toUpperCase() ) ){
				customTab.addContentsTabbyObj( tabPage.caption, divContents );
			}else{
...
/** Add By DW KangDoY Single LayOut
 * jQueryLayout
 * @param		targetObj : PosLayout을 추가할 대상 객체
 * @param		arrPaneNames : 사용할 pane 이름 배열. center
 * @param		options : Layout에 적용할 옵션
 * @param		topMargin : Layout 상위 margin
 * @returns
 * @method		getLayout : Return Layout Object
 ...
 * @example
 */
function jQuerySingleLayout( targetObj, arrPanelNms, options, topMargin ) {
	var paneNames = ['center' ];
	var divLayout = null, objLayout = null;
...
function fc_linkageTab( pageId, arrParams ) {
	...
	var mnu_id = '';
	// arrParam에 MNU_ID가 있을경우 해당 MEMU로 분기
	for ( var i=0;i<arrParams.length;i++ ) {
		if(arrParams[ i ].name == "MNU_ID") {
			mnu_id = arrParams[ i ].value; 
		}
	}
	//
	...
	$.each( treeInst._model.data, function( index, item ) {
		...
		if ( arrObj.length != 0 ) {
			// arrParam에 MNU_ID가 있을경우 해당 MEMU로 분기
			if(mnu_id == '' || mnu_id == arrObj[0]['MNU_ID']) {
				return false;
			}
			//
		}
	...
	try {
		...
		parent.mainTab.openTabByObj( targetMenu, paramString, true );
	} catch ( e ) {
		fc_getException( e );
	};
...
function fc_linkagePopup( pageId, arrParams, nWidth, nHeight, isModal ) {
	...
	try {
		...
		var targetPage = '/T/' + pageId + paramString;
...
function f_makePopupAttr( nWidth, nHeight, isModal ) {
	try {
		...
		// 20200217 
		// 팝업리사이즈 변경 resizable : false  --> true
		// 최대사이즈 변경 
...
//	window.gwMesEnv.operInfo.dateTime = new Date( window.gwJsonResult.RK_OPDATE[ 0 ].CURR_DATE_TIME );
	//KD 0723 Change Date Type new Date("yyyy-mm-dd hh:mm:ss") -> new Date(year, month, date, hours, min, sec)
	var arrTimeStr = window.gwJsonResult.RK_OPDATE[ 0 ].CURR_DATE_TIME.substring(11).split(':');
	window.gwMesEnv.operInfo.dateTime = new Date( arrStr[ 0 ], arrStr[ 1 ] - 1, arrStr[ 2 ], arrTimeStr[0], arrTimeStr[1], arrTimeStr[2]);
	//KD ~
...
/**
 * fc_getUserDate
 * Create By Kang Dong Young
 * @returns
 */
function fc_getUserDate(day) {
	if(fc_isNull(day)){
		day = 0;
	}
	var nowDate = fc_getNow();
	var date = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate()+day);
	return date;
}; // end of fc_getNow
/**
 * fc_getUserDateTime
 ...
 * @returns
 */
function fc_getUserDateTime(day, hour, min, sec) {
	if(fc_isNull(day)){
		day = 0;
	}
	var nowDate = fc_getNow();
	var date = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate()+day, nowDate.getHours()+hour, nowDate.getMinutes()+min, nowDate.getSeconds()+sec);
	return date;
}; // end of fc_getNow
/**
 * fc_getCurrentDate
...
function fc_removePageableInGrid( arrGrid ) {
	for ( var loop = 0; loop<arrGrid.length; loop++ ) {
		...
		$( '#' + arrGrid[ loop ] + ' #pager' + arrGrid[ loop ] ).css('display','none');
 ...
 * @param bCust5
 */
function fc_setDisableForCommonBtn( flag, bConf, bCust1, bCust2, bCust3, bCust4, bCust5, bCust6, bCust7, bCust8, bCust9, bCust10 ) {
	...
	if ( !fc_isNull( bCust6 ) ) $( '#' + window.gwBtn.cust6Btn   ).jqxButton( { disabled: !bCust6 } );
	if ( !fc_isNull( bCust7 ) ) $( '#' + window.gwBtn.cust7Btn   ).jqxButton( { disabled: !bCust7 } );
	if ( !fc_isNull( bCust8 ) ) $( '#' + window.gwBtn.cust8Btn   ).jqxButton( { disabled: !bCust8 } );
	if ( !fc_isNull( bCust9 ) ) $( '#' + window.gwBtn.cust9Btn   ).jqxButton( { disabled: !bCust9 } );
	if ( !fc_isNull( bCust10 ) ) $( '#' + window.gwBtn.cust10Btn   ).jqxButton( { disabled: !bCust10 } );
...
function fc_showCommonBtns( arrBtnNm, flag ) {
	...
	for ( var loop=0;loop<arrBtnNm.length;loop++ ) {
		...
		case 'CUST6' :
			...
			else $( '#' + window.gwBtn.cust6Btn ).hide();
			break;
		case 'CUST7' :
			if ( sAuth && flag )
				 $( '#' + window.gwBtn.cust7Btn ).show();
			else $( '#' + window.gwBtn.cust7Btn ).hide();
			break;
		case 'CUST8' :
			if ( sAuth && flag )
				 $( '#' + window.gwBtn.cust8Btn ).show();
			else $( '#' + window.gwBtn.cust8Btn ).hide();
			break;
		case 'CUST9' :
			if ( sAuth && flag )
				 $( '#' + window.gwBtn.cust9Btn ).show();
			else $( '#' + window.gwBtn.cust9Btn ).hide();
			break;
		case 'CUST10' :
			if ( sAuth && flag )
				 $( '#' + window.gwBtn.cust10Btn ).show();
			else $( '#' + window.gwBtn.cust10Btn ).hide();
			break;
		default :
			break;
		};
...
function fc_setVisibleForCommonBtn( bSave, bDelete, bConf, bCust1, bCust2, bCust3, bCust4, bCust5, bCust6, bCust7, bCust8, bCust9, bCust10 ) {
	...
	if ( bCust6 )	$( '#' + window.gwBtn.cust6Btn ).show();
	else			$( '#' + window.gwBtn.cust6Btn ).hide();

	if ( bCust7 )	$( '#' + window.gwBtn.cust7Btn ).show();
	else			$( '#' + window.gwBtn.cust7Btn ).hide();

	if ( bCust8 )	$( '#' + window.gwBtn.cust8Btn ).show();
	else			$( '#' + window.gwBtn.cust8Btn ).hide();

	if ( bCust9 )	$( '#' + window.gwBtn.cust9Btn ).show();
	else			$( '#' + window.gwBtn.cust9Btn ).hide();

	if ( bCust10 )	$( '#' + window.gwBtn.cust10Btn ).show();
	else			$( '#' + window.gwBtn.cust10Btn ).hide();
...
function fc_clearFormData( pId ) {
	$( '#' + pId ).find( '.formValueCheck' ).each( function () {
		if($( this ).prop('datatype') == 'checkbox'){
			fc_setInputVal( $( this ).prop( 'id' ), false );
		}else{
		...
		for ( var i=0; i < arrCodeData.length; i++ ) {
			...
			if ( fc_isNull( arrowVal ) && parentNullOpt ) {
				...
				if ( !fc_isNull( sCodeResultArrowColNm ) ) {
					...
					if(typeof arrowObj == 'number') {
					    if ( arrowVal == arrowObj ) {
					        arrNewCodeObj.push(arrCodeData[ i ]);
					    }
					} else {
...
function fc_lovClickEvent(id, eventCall){
	try{
		var obj = fc_getObj(id);
...
$( window ).keydown( function ( event ) {
	...
	if ( event.keyCode == 13 ) {
		...
		$.each( node, function ( index, item ) {
			...
			}else if( item.id == 'popupWindowCondition'){
				
				// 20191127 
				// popup화면에서 검색조건 엔터시 상위화면에 걸려있는 엔터키 이벤트가 발생하여 포커스 이동하고 엔터 이벤트를 발생시켜줌 
				$("#"+event.target.id).focus();
				var e = jQuery.Event( "keypress", { keyCode: 13 } ); 
				$("#"+event.target.id).trigger(e);
...
$( window ).mousedown( function ( event ) {
	...
	if ( event.target.id != 'collapseButton' ) {
		if ( $( event.target ).parents().filter( function( index ) {
			...
			if ($( event.target )[ 1 ].outerText != null && ($( event.target )[ 0 ].outerText.trim() == window.gwMessage.fav.add || $( event.target )[ 0 ].outerText.trim() == window.gwMessage.fav.remove && $( event.target )[ 0 ].outerText.trim() == window.gwMessage.fav.def) ) {
				null;
			} else {
				try {
