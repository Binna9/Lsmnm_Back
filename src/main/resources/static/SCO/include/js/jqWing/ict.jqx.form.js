...
function fc_createElement( targetObj, elementObj, type ) {
	try {
		...
		var inputAttObj =  {
				...
				caption    : _id,
				width      : 100,
				height     : ( fc_isNull( elementObj.height ) ) ? 26 : elementObj.height,
				datatype   : 'text',
				align      : 'left',
				readonly   : false,
				itemCd     : _id,
...
function fc_createColElement( targetObj,  elementObj, screenType ) {
	try {
		...
		var inputAttObj =  {
				...
				caption    : _id,
				width      : 100,
				height     : ( fc_isNull( elementObj.height ) ) ? 26 : elementObj.height, // 24
				datatype   : 'text',
				align      : 'left',
				readonly   : false,
				itemCd     : _id,
...
function fc_createTH( prmObj, inputAttObj ) {
	...
	try {
		...
		var labelObj = { name: '', id: '' };
		labelObj.name = inputAttObj.name;
		// 20191121
		// Form Table의 Header 객체 ID 부여
		labelObj.id = "formTableHeader_"+inputAttObj.id;
...
function fc_createTD( prmObj, inputAttObj ) {
	try {
		...
		var tdElObj	 = { 'class' : 'form-td', 'align' : 'center' };
...
function fc_setFormNumberInput ( numberId, option ) {
	try {
		...
		console.log('nDecimalDigits:', nDecimalDigits, 'target.decimal:', window.gwMesEnv.format.separate.target.decimal, 'target.thousand:', window.gwMesEnv.format.separate.target.thousand);
		
		$( '#' + numberId ).number( option.digitsChk
				                  , nDecimalDigits
...
function fc_setFormDateTime( dateInputId, option ) {
	try {
		...
		/*
		 * not working at IE 11
		$( '#' + dateInputId ).jqxDateTimeInput( 'setDate', null );
		if ( !fc_isNull( option.refName ) && !isFrom ) {
			var objFr = $( '#' + option.refName ); //start
			var objTo = $( '#' + dateInputId ); //end

			objFr.on( 'change', function ( event ) {
				var valFr = objFr.jqxDateTimeInput( 'getDate' );
				var valTo = (objTo.jqxDateTimeInput( 'getDate' )==null) ? valFr:objTo.jqxDateTimeInput( 'getDate' );
 ...
 * @param toNum
 */
function fc_setSearchDateInit(startId, endId, fromNum , toNum){
	fromNum = typeof fromNum !== 'undefined' ? fromNum : 0;
	toNum = typeof toNum !== 'undefined' ? toNum : 0;
	...
	try {
		...
		window.onload = function(){
			$( '#'+startId  ).jqxDateTimeInput( 'setDate', new Date( fc_getNow().getFullYear(), fc_getNow().getMonth(), fc_getNow().getDate()+fromNum ) );
			$( '#'+endId   	).jqxDateTimeInput( 'setDate', new Date( fc_getNow().getFullYear(), fc_getNow().getMonth(), fc_getNow().getDate()+toNum ) );
		};
	} catch (e) {
		fc_getException( e );
	}
 ...
 * @param listCount
 */
function fc_setFormDropdownListData( editId, arrObj, listCount, isSelectMax, datatype, dropDownHeight ) {
	try {
		var lovSource = {
			datatype: 'array',
			datafields: [
		...
		var rowHeight = 25;
		if(!fc_isNull(datatype) && fc_checkDatatype(datatype,'chklov')){
			rowHeight = 26;
		}
		
		
		
		var heightSize = arrObj.length * rowHeight + 2 ;
		if ( !fc_isNull( listCount ) ) {
			heightSize = listCount * rowHeight + 2;
		};
		if ( heightSize > 600 ) {
			heightSize = 600;
		};
		...
		// 20200427
		if( dropDownHeight != undefined && !fc_isNull( dropDownHeight )){
			heightSize = dropDownHeight;
		}
		...
		if ( checkboxes ) {
			...
			if(isSelectMax){//2019.12.6
				$( '#' + editId ).jqxDropDownList( 'selectedIndex', lovAdapter._source.localdata.length-1 );
			}else{
...
function fc_setFormComboboxData( editId, arrObj, listCount ) {
	try {
		...
		var lovAdapter = new $.jqx.dataAdapter( lovSource, { autoBind: true } );
		var heightSize = arrObj.length * 26;

		if ( !fc_isNull( listCount ) ) {
			heightSize = listCount * 26;
		};
		if ( heightSize > 250 ) {
			heightSize = 250;
		};
...
function fc_setDropDownListBoxWidth( editObj ) {
	...
	dropdownMexWidth = dropdownMexWidth + 25;
...
function fc_setInputVal( targetId, value ) {
	try {
		...
		//datatype="hourmin"
		//console.log('$$$$$'+catagory + ' : ' + (typeof value))
		...
		if ( catagory === 'date' && typeof value === 'date' ) {
			...
			var _value = null;
			if($( '#' + targetId ).attr( 'datatype' ) == 'hourmin'){
				_value = value;
			}else{
				try {
		...
		} else if ( catagory == 'checkbox' ) {
			if ( fc_isNull( value ) || value == false || value == 'false' ) {
				$( '#' + targetId ).val( false );
			} else {
				$( '#' + targetId ).val( true );
			};
...
function fc_setHideElement( arrTargetId, isHide ) {
	if ( isHide ) {
		for ( var iLoop=0;iLoop<arrTargetId.length;iLoop++ ) {
			$( '#divgrp_' + arrTargetId[ iLoop ].toUpperCase() ).hide();
		};
	} else {
		for ( var iLoop=0;iLoop<arrTargetId.length;iLoop++ ) {
			$( '#divgrp_' + arrTargetId[ iLoop ].toUpperCase() ).show();
		};
	};
